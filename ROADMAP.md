...
- [ ] Wire anomaly detection, adaptive restart, and resource prediction outcomes into the runtime lifecycle with durable per-container persistence. Anomaly, predictive-OOM, and adaptive-restart decisions now use a shared durable JSONL logger under the configured runtime directory by default (or `FERROCRATE_AI_AUDIT_LOG`), successful adaptive restarts emit `ai_restart_applied` receipts with delay/PID evidence, and bounded `ai_restart_failed` receipts now record command-build, spawn, PID-state, and ownership-release failures; adaptive restart honors an AI `DoNotRestart` decision and is disabled by `FERROCRATE_AI=0`, while reconciliation and cross-container action persistence remain open.
