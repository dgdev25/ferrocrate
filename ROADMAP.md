...
- [ ] Complete high-value Docker container, image, network, volume, streaming, wait, and event endpoints while preserving authorization and witness receipts; authorized image tagging, network/volume inspect/list/prune, `/containers/{id}/wait` timeout, `/containers/{id}/logs` `tail`/chunked `follow=1`, and authorized image pruning are wired, while broader streaming and endpoint parity remain open.
