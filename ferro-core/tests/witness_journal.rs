...
#[test]
fn rejects_legacy_storage_directory_at_open() {
    let dir = tempdir().unwrap();
    std::fs::create_dir_all(dir.path().join("witness.sled")).unwrap();
    assert!(matches!(
...
fn outcome_unknown_remains_pending_until_linked_recovery() {
    ...
    // Outcome telemetry is process-global; this integration test runs in
    // parallel with other legitimate unknown-outcome fixtures. The durable
    // pending/recovery contract below is the behavior owned by this test.
