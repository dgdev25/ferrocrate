...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `aa2675a`
...
- [ ] Ship rootless installer and upgrade flows for subordinate IDs, mapping helpers, cgroup delegation, user services, and slirp networking; `scripts/rootless-install.sh` now provides non-privileged install and explicit `--upgrade` mode with atomic binary/unit replacement, subordinate-ID/cgroup-v2/user-namespace/runtime-dir diagnostics, strict fail-before-mutation gating, dry-run mode, collision checks, systemd-unit path validation, and trust checks rejecting symlinked, non-root-owned, or writable helper binaries, with repeatable coverage in `scripts/test-rootless-install.sh`; rootless bridge setup now binds the slirp helper to exact PID/start-time rollback cleanup (`docs/evidence/verification/2026-08-16-rootless-slirp-rollback.md`), while helper provisioning and full networking qualification remain open.
- [ ] Add rootless contexts, socket discovery, diagnostics, logs, and explicit unsupported-feature messaging; Linux doctor now reports mapping/socket context, accepts only actual Unix sockets (rejecting regular-file/symlink decoys), and the CLI persists validated named contexts with context inspect availability reporting. Selected remote Unix-socket contexts now route `run`, images, pull/rmi/image-prune, non-interactive exec, push, containers, inspect, logs, non-streaming stats, network/volume listing and inspect, network/volume create and remove, pause, unpause, stop, kill, restart, and container remove through the Docker-compatible transport (docs/evidence/verification/2026-08-16-remote-run-routing.md; docs/evidence/verification/2026-08-16-remote-context-routing.md; docs/evidence/verification/2026-08-16-remote-image-routing.md; docs/evidence/verification/2026-08-16-remote-exec-routing.md; docs/evidence/verification/2026-08-16-remote-push-routing.md); other mutating/streaming daemon mappings and full rootless qualification remain open.
