...
fn server() -> server::NetdServer {
    ...
    server::NetdServer::new(
        1001,
        policy::Policy::new("cluster".into(), "node".into(), &key).unwrap(),
    )
}
...
fn rejects_invalid_signature() {
    ...
    let request = SignedEnvelope {
        ...
        epoch: 1,
        revision: 1,
        lease_expires_unix_secs: 2,
        request: NetdRequest::Inspect {
            overlay_id: "wg0".into(),
        },
        signature: "AAAA".into(),
    };
    ...
    frame.extend(body);
    assert_eq!(
        server.handle_peer(1001, &frame, 1).code(),
        Some(RejectionCode::InvalidSignature)
    );
}
...
fn rejects_direct_peer_without_request_bound_grant() {
    ...
    envelope.signature = base64::engine::general_purpose::STANDARD.encode(
        ed25519_dalek::Signer::sign(&signing, &serde_json::to_vec(&envelope).unwrap()).to_bytes(),
    );
    ...
    assert_eq!(
        server().handle_peer(1001, &frame, 1).code(),
        Some(RejectionCode::MissingGrant)
    );
}
...
fn policy_revision_can_be_rolled_back_for_retry() {
    ...
    envelope.signature = base64::engine::general_purpose::STANDARD
        .encode(ed25519_dalek::Signer::sign(&signing, &unsigned).to_bytes());
...
fn policy_allows_same_revision_only_when_lease_is_extended() {
    ...
    assert_eq!(
        policy.validate(&signed_envelope(&signing, 100), 1),
        Err(RejectionCode::StaleRevision)
    );
