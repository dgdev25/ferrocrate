...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `b8a4209`
...
- [ ] Add rootless contexts, socket discovery, diagnostics, logs, and explicit unsupported-feature messaging; Linux doctor now reports mapping/socket context, accepts only actual Unix sockets (rejecting regular-file/symlink decoys), and the CLI persists validated named contexts with context inspect availability reporting. Selected remote Unix-socket contexts now route images, containers, inspect, logs, non-streaming stats, network/volume listing and inspect, network/volume create and remove, pause, unpause, stop, kill, restart, and container remove through the Docker-compatible transport (docs/evidence/verification/2026-08-16-remote-context-routing.md); other mutating/streaming daemon mappings and full rootless qualification remain open.
...
- [ ] Define and implement an opt-in `.rvf` image manifest, build format, launcher, and OCI interoperability. The versioned manifest/build format is implemented, and `ferro-core::rvf_image::read_rvf_image` plus atomic `extract_layer` now provide a digest- and size-validated OCI layer handoff; `ferrocrate rvf import` now validates platform/digest, publishes layer/config blobs atomically, and writes an authorization-bound OCI reference (`docs/evidence/verification/2026-08-16-rvf-import.md`), while native launcher/QEMU execution and direct `run` wiring remain open.
