...
require_text "$macos" 'VM_KNOWN_HOSTS_PATH=.*known_hosts' 'per-VM SSH host-key cache'
require_text "$macos" 'StrictHostKeyChecking=accept-new' 'fresh guest host-key acceptance'
require_text "$macos" 'ssh-private-key-path' 'recorded SSH key path'
