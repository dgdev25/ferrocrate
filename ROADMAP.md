...
- [ ] Complete high-value Docker container, image, network, volume, streaming, wait, and event endpoints while preserving authorization and witness receipts; `/containers/{id}/wait` now honors the Docker timeout query (including zero=infinite), while broader streaming and endpoint parity remain open.
