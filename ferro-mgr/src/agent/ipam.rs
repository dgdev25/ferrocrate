...
impl Ipam {
    pub fn new(
        ...
        if !pool.contains(&gateway) || gateway == pool.network() || gateway == pool.broadcast() {
            return Err(IpamError::InvalidPool);
        }
        Ok(Self {
            pool,
            gateway,
    ...
    pub fn with_state(
        ...
        if path.exists() {
            let metadata = fs::symlink_metadata(&path)
                .map_err(|error| IpamError::Persistence(error.to_string()))?;
            if !metadata.file_type().is_file()
                || metadata.mode() & 0o077 != 0
                || metadata.uid() != nix::unistd::geteuid().as_raw()
            {
                return Err(IpamError::Persistence("insecure IPAM state path".into()));
            }
            let bytes =
                fs::read(&path).map_err(|error| IpamError::Persistence(error.to_string()))?;
            instance.allocations = Mutex::new(
    ...
    pub fn allocate(&self, container_id: impl Into<String>) -> Result<Allocation, IpamError> {
        ...
        if allocations.contains_key(&container_id) {
            return Err(IpamError::DuplicateContainer);
        }
        ...
        for value in start..end {
            ...
            if let Err(error) = self.persist(&allocations) {
    ...
    pub fn release(&self, container_id: &str) -> Result<Option<Allocation>, IpamError> {
        let mut allocations = self
            .allocations
            .lock()
            .map_err(|_| IpamError::Persistence("IPAM lock poisoned".into()))?;
        let allocation = allocations.remove(container_id).map(|address| Allocation {
            container_id: container_id.into(),
            address,
        });
        if let Err(error) = self.persist(&allocations) {
            if let Some(allocation) = &allocation {
                allocations.insert(allocation.container_id.clone(), allocation.address);
            }
            return Err(error);
        }
        Ok(allocation)
