...
function global:signtool.exe {
  $script:calls += $args[0]
  $global:LASTEXITCODE = if ($args[0] -eq $script:failOperation) { 23 } else { 0 }
}
try {
  foreach ($operation in @('sign', 'verify', 'none')) {
    ...
    if ($operation -eq 'sign' -and $script:calls -contains 'verify') { throw 'Verification ran after signing failed' }
    if ($operation -eq 'none' -and ($script:calls -join ',') -ne 'sign,verify') { throw 'Success did not sign then verify' }
