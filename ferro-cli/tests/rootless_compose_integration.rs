...
fn rootless_compose_services_share_the_project_network_namespace() {
    if std::env::var("FERROCRATE_RUN_ROOTLESS_E2E").as_deref() != Ok("1")
        || std::env::var("FERROCRATE_RUN_ROOTLESS_SHARED_NETWORK").as_deref() != Ok("1")
    {
        return;
    }
    ...
    assert!(
        up.status.success(),
        "shared-network compose up failed: {}",
    ...
    if !workspace.join("output").exists() {
        ...
        panic!(
            "shared-network follower did not reach leader: stdout={} stderr={}",
    ...
    assert!(
        down.status.success(),
        "shared-network compose down failed: {}",
        String::from_utf8_lossy(&down.stderr)
    );
    assert_eq!(
        fs::read_to_string(workspace.join("output")).expect("shared output"),
        "shared-network\n"
    );
}
