...
- [ ] Complete the remaining runtime shell-out audit; the current production inventory and justified exceptions are documented in `docs/security/shell-out-audit.md`, and WireGuard now uses the shared typed executor, while eBPF/tc executor convergence and kernel qualification remain open.
