...
- [ ] Add plugin identity, signature/permission, lifecycle, retry, isolation, and resource limits; v1 now verifies canonical Ed25519 signatures, allow-listed permissions, bounded memory/PID/timeout/output declarations, owner-only trust-root loading, and a shell-free executor applying Unix address-space limits plus timeout/output bounds. An explicit cgroup-v2 executor now applies memory/PID ceilings and kills/cleans descendants, failing closed when a real hierarchy is unavailable; host qualification, retries, and durable lifecycle provenance remain open.
