...
- [ ] S8: Split `docs/` into public and internal — move agent process history, task lists, handoffs, design mockups and business plans to a gitignored `docs/internal/`; keep user guides, ADRs, security, operations, compatibility, platform acceptance and evidence reports; drop raw evidence dumps not linked from a report; repoint README/SECURITY links. Blocked on the user's approval of the proposed split.
