...
| Windows 11 via WSL2 backend | Host/network-blocked boundary: `socat` is now present and native/Tauri artifacts were built on `E:`, but the WSL engine build and later host network outage prevented bridge acceptance | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): stale refused socket and exact SSH `No route to host`; no Doctor, token, or browser/runtime claim |
| macOS Tahoe via Linux VM backend | Host/network-blocked boundary: nested HV, Node/npm, and QEMU are now available and a fresh build was launched, but an SSH network outage prevented VM/bridge acceptance | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): exact SSH `No route to host`; no VM/runtime claim |
