...
| Windows 11 via WSL2 backend | Acceptance blocked by a supervisor/WSL relay defect: a guest daemon responds on its Unix socket, but the Windows supervisor cannot recognize it and attempts a second engine | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): SSH restored; exact repeated “runtime engine is already owned” evidence; no Doctor, token, or browser/runtime claim |
| macOS Tahoe via Linux VM backend | Qualified host boundary: nested HV, Node/npm, and QEMU are available, but an independently owned guest Cargo build prevents a compliant acceptance run | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): SSH restored; one-Cargo-process rule preserved; no VM/runtime claim |
