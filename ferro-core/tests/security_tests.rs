...
fn applies_profile_configured_errno_return() {
    ...
    if child == 0 {
        ...
        let profile = ferro_core::seccomp::SeccompProfile {
            ...
            syscalls: vec![ferro_core::seccomp::SyscallRule {
                names: vec!["write".to_string(), "exit".to_string(), "exit_group".to_string()],
                action: "SCMP_ACT_ALLOW".to_string(),
                errno_ret: None,
                args: None,
            }],
    ...
    let mut marker = [0_u8];
    assert_eq!(unsafe { nix::libc::read(pipe_fds[0], marker.as_mut_ptr().cast(), 1) }, 1);
    unsafe { nix::libc::close(pipe_fds[0]) };
    let mut status = 0;
    assert_eq!(unsafe { nix::libc::waitpid(child, &mut status, 0) }, child);
    assert_eq!(status, 0);
    assert_eq!(marker[0], 1, "configured errno should be returned by seccomp");
}
