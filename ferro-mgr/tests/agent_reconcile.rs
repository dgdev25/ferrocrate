...
fn failed_netd_apply_does_not_advance_persisted_state() {
    ...
    let agent = Agent::new("cluster-a", vec![4; 32], StateStore::new(directory.path().join("state.json")), FakeNetd { calls: calls.clone(), fail: true }).unwrap();
...
fn successful_reconcile_persists_revision_after_netd() {
    ...
    let calls = Arc::new(AtomicUsize::new(0));
    let agent = Agent::new("cluster-a", vec![4; 32], StateStore::new(&path), FakeNetd { calls: calls.clone(), fail: false }).unwrap();
