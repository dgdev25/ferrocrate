...
- [ ] Test durable sandbox/container recovery after process termination at each intent/effect/store boundary; CRI metadata now uses transactional SQLite with legacy JSON import, no-root reopen coverage verifies persisted `Exited` state, exit code, and reason, and the real OCI socket fixture now restarts the CRI daemon while a workload is running and verifies recovered identity/state before continuing. Hard process-kill coverage at every intent/effect/store boundary remains open.
