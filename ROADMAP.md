...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `dcd3a0d`
...
- [ ] Add rootless contexts, socket discovery, diagnostics, logs, and explicit unsupported-feature messaging; Linux doctor now reports mapping/socket context, accepts only actual Unix sockets (rejecting regular-file/symlink decoys), and the CLI persists validated named contexts with context inspect availability reporting. Selected remote Unix-socket contexts now route images, containers, inspect, logs, and non-streaming stats through the Docker-compatible transport (docs/evidence/verification/2026-08-16-remote-context-routing.md); mutating/streaming daemon mappings and full rootless qualification remain open.
