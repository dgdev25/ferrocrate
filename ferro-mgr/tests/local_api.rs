...
fn local_api_requires_uid_and_current_lease() {
    ...
    assert_eq!(
        api.attach(2002, "overlay", "c1", 100),
        Err(LocalApiError::Unauthorized)
    );
    assert_eq!(
        api.attach(1001, "overlay", "c1", 200),
        Err(LocalApiError::Expired)
    );
...
fn ipam_state_survives_restart() {
    ...
    assert_eq!(
        first.allocate("c1").unwrap().address.to_string(),
        "10.3.0.2"
    );
    drop(first);
    let second = Ipam::with_state(pool, "10.3.0.1".parse().unwrap(), vec![], path).unwrap();
    assert_eq!(
        second.allocate("c2").unwrap().address.to_string(),
        "10.3.0.3"
    );
}
...
fn local_api_rejects_unknown_overlay_after_authorization_registry_is_enabled() {
    let ipam = Ipam::new(
        "10.4.0.0/29".parse().unwrap(),
        "10.4.0.1".parse().unwrap(),
        vec![],
    )
    .unwrap();
    let api = LocalApi::new(1001, 200, ipam);
    api.register_overlay(
        "managed",
        OverlayConfig {
            bridge: "fc-managed".into(),
            gateway: "10.4.0.1".parse().unwrap(),
            prefix: 24,
            mtu: 1420,
        },
    )
    .unwrap();
    assert_eq!(
        api.attach(1001, "other", "c1", 100),
        Err(LocalApiError::UnknownOverlay)
    );
