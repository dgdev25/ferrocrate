...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `244ef20`
...
- [ ] Provide a complete durable, filterable event stream distinct from the witness journal; the separate fsynced JSONL stream now projects Docker's core event wire shape with nanosecond timestamps, persisted method/path/status/scope actor attributes, create-response resource IDs, bounded request labels/identity fields, label selectors, rejects corrupt journal records and invalid time bounds, avoids fabricating collection-operation names as resource IDs, supports type/action/scope/time/container/image/network/volume resource filters and Docker's JSON `filters` query form, and provides `/events` socket coverage including chunked `follow=1` streaming until client disconnect (`docs/evidence/verification/2026-08-16-docker-event-actor-identity.md`); broader event-attribute parity remains open.
