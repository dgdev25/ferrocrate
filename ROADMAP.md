...
- [ ] Add rootless contexts, socket discovery, diagnostics, logs, and explicit unsupported-feature messaging; Linux doctor now reports the mapping and socket context with unsupported CRI/Compose/advanced-network boundaries, while dedicated context management and log streaming remain open.
...
- [ ] Provide a complete durable, filterable event stream distinct from the witness journal; the separate fsynced JSONL stream now projects Docker's core event wire shape with type/action/time/resource filters and `/events` socket coverage, while live-follow semantics and full event-attribute parity remain open.
