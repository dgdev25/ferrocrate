...
use crate::{
    config::MAX_PENDING_REVISIONS,
    proto::{AgentMessage, DesiredState, ManagerMessage},
};
...
impl ControlServiceImpl {
    ...
    pub fn connect(
        ...
        cluster_id: &str,
        epoch: u64,
    ) -> Result<ControlSession<'_>, ControlError> {
        if cluster_id != self.cluster_id {
            return Err(ControlError::WrongCluster);
        }
        ...
        let mut active = self
            .active_nodes
            .lock()
            .expect("control session lock poisoned");
...
impl<'a> ControlSession<'a> {
    pub fn receive(&self, message: &AgentMessage) -> Result<Option<ManagerMessage>, ControlError> {
        ...
        if message.acknowledged_revision > latest {
            return Err(ControlError::FutureRevision);
        }
        if message.payload.len() > crate::config::MAX_MESSAGE_BYTES {
...
impl Drop for ControlSession<'_> {
    fn drop(&mut self) {
        if let Ok(mut active) = self.service.active_nodes.lock() {
...
#[allow(dead_code)]
fn _typed_state(state: DesiredState) -> ManagerMessage {
    ManagerMessage {
