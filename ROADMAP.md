...
- [ ] Wire anomaly detection, adaptive restart, and resource prediction outcomes into the runtime lifecycle with durable per-container persistence. Anomaly, predictive-OOM, and adaptive-restart decisions now use a shared durable JSONL logger under the configured runtime directory by default (or `FERROCRATE_AI_AUDIT_LOG`), serialize concurrent container appenders under one process-wide lock, and sync the parent directory after each record; successful adaptive restarts emit `ai_restart_applied` receipts with delay/PID evidence, bounded `ai_restart_failed` receipts record command-build, spawn, PID-state, and ownership-release failures, and stale-PID recovery emits `ai_reconciliation_applied`; adaptive restart honors an AI `DoNotRestart` decision and is disabled by `FERROCRATE_AI=0`, while cross-process and kernel-effect reconciliation persistence remain open.
