...
use ferro_mgr::{
    ...
    proto::{AgentMessage, OverlayState},
};
...
fn duplicate_sessions_and_revision_gaps_are_bounded() {
    ...
    assert!(matches!(
        service.connect("node-a", "cluster-a", 4),
        Err(ControlError::DuplicateSession)
    ));
    ...
    assert_eq!(
        session.receive(&message).unwrap_err(),
