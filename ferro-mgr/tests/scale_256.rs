...
struct NoopNetd;
impl NetdClient for NoopNetd {
    fn apply(&self, _: &DesiredState) -> Result<(), String> {
        Ok(())
    }
}
...
fn agents_256_converge_to_one_revision_without_secret_metrics() {
    ...
    for index in 0..256 {
        let agent = Agent::new(
            ...
            SigningKey::from_bytes(&[5; 32])
                ...
                .to_bytes()
                .to_vec(),
            StateStore::new(directory.path().join(format!("agent-{index}.json"))),
            NoopNetd,
        )
        .unwrap();
        agents.push(agent);
    ...
    for agent in &agents {
        assert_eq!(agent.reconcile(desired.clone(), 101).unwrap(), 42);
    }
    assert!(agents
        .iter()
        .all(|agent| agent.state().applied_revision == 42));
    let metrics = ManagerMetrics {
        connected_nodes: 256,
        desired_revision: 42,
        applied_revision: 42,
        rotation_state: "Committed".into(),
        ..Default::default()
    };
