...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `123831f`
...
- [ ] Replace the remaining `sled` backends to remove `RUSTSEC-2025-0057` (`fxhash`) and `RUSTSEC-2024-0384` (`instant`), including migration and backward-compatibility tests. Volume, image, CRI metadata/replay, Compose replay, the active container runtime, and the witness journal now use SQLite at runtime; the witness journal has tested atomic multi-tree cutover, legacy v2 import with a readiness marker, reopen/fault coverage, and a retained sled compatibility importer. The dependency-removal step still requires moving that importer out of the production dependency graph. The schema, transaction contract, locked cutover, retry behavior, and verification gates are specified in `docs/architecture/witness-journal-sqlite-migration.md`; the post-cutover workspace run is archived in `docs/evidence/verification/2026-08-16-workspace-serial.md`.
