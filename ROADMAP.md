...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `c1c4dea`
...
- [ ] Test durable sandbox/container recovery after process termination at each intent/effect/store boundary; CRI metadata now uses transactional SQLite with WAL/`synchronous=FULL` durability and legacy JSON import, no-root reopen coverage verifies persisted `Exited` state, exit code, and reason, startup reconciles a missing CRI runtime binding from the stable runtime label plus sandbox parent and fails closed on ambiguity, the real OCI socket fixture restarts the CRI daemon while a workload is running and verifies recovered identity/state, subprocess fixtures now hard-kill the actual `ferro-cri` binary after each durable sandbox/container lifecycle transition (`docs/evidence/verification/2026-08-16-cri-operation-recovery.md`), the complete CRI test suite and Clippy gate pass on the qualified Ubuntu host (`docs/evidence/verification/2026-08-16-cri-test-suite.md`), and sandbox/container mutations now roll back in-memory metadata and newly created runtime/netns effects when durable publication fails. Mid-operation fault injection and broader kernel-effect reconciliation remain open.
...
- [ ] Qualify rootless CRI, Compose, volumes, networking, image signing, and resource limits; the 2026-08-16 run now passes user namespaces, subordinate-ID helpers, cgroup v2, runtime-dir checks, `slirp4netns`, and the real namespace mapping fixture, while workload-level networking/CRI/Compose qualification remains open (`docs/evidence/rootless/2026-08-16-run.md`).
