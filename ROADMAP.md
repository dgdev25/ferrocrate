...
- [ ] Complete high-value Docker container, image, network, volume, streaming, wait, and event endpoints while preserving authorization and witness receipts; authorized image tagging, network/volume inspect/list/prune, `/containers/{id}/wait` timeout, `/containers/{id}/logs` `tail`/chunked `follow=1`, and chunked `/containers/{id}/stats?stream=1` are wired, while broader streaming and endpoint parity remain open.
...
- [ ] Add deterministic cache keys, cache import/export, remote/registry cache, pruning, and cache provenance; local cache entries now persist their key and creation timestamp and are written atomically, while import/export, remote cache, pruning, and graph-wide provenance remain open.
...
- [ ] Add plugin identity, signature/permission, lifecycle, retry, isolation, and resource limits; v1 now verifies canonical Ed25519 signatures, allow-listed permissions, and bounded memory/PID/timeout/output limits, while trust-root selection, lifecycle, and actual sandbox enforcement remain open.
