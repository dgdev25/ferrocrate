...
| Windows 11 via WSL2 backend | Acceptance blocked by a supervisor/UI double-owner defect: the direct WSL guest daemon is healthy, but the requested supervisor-plus-web sequence starts a second engine owner | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): re-attempt 4; refused-socket and ownership evidence; no Doctor, token, or browser/runtime claim |
| macOS Tahoe via Linux VM backend | Host bridge blocked: nested HV, Node/npm, and QEMU/HVF work and the QEMU guest starts, but macOS OpenSSH rejects the accepted guest key | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): re-attempt 4; no bridge/runtime claim |
