...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `9e776de`
...
- [ ] Complete high-value Docker container, image, network, volume, streaming, wait, and event endpoints while preserving authorization and witness receipts; image history, container list `all`/`limit`/`since`/`before` filters with deterministic newest-first ordering, image-list reference/time filters, authorized image tagging, image-prune `dangling`/`until` filters with strict unsupported-selector rejection, network/volume inspect/list/prune (network list/prune support `name`/`driver`/`scope`/`type`, Docker network create now separates IPv4 and IPv6 IPAM entries and validates `EnableIPv6`, and network list/inspect now preserve the requested IPv6 subnet/gateway in Docker IPAM responses; volume list/prune support exact `name`/`driver`, all with fail-closed unsupported selectors), authorized container rename with durable metadata mutation, `/containers/{id}/wait` timeout with validated `not-running`/`next-exit` conditions, pause/unpause, `/containers/{id}/top`, logs `tail`/chunked `follow=1`, `/containers/{id}/attach` strict query validation plus explicit TCP upgrade/raw follow support and stdin forwarding when a live container's `/proc/<pid>/fd/0` is available (`docs/evidence/verification/2026-08-16-docker-attach.md`), chunked stats streaming, health state in container inspect, and the two-step authorized `/containers/{id}/exec` plus `/exec/{id}/start` API are wired; rejected and valid create payloads, pre-start inspect/list, atomic pending-create reload, and pending removal are covered by evidence, while real OCI start/inspect execution qualification, cross-host streams, and broader endpoint parity remain open.
...
- [ ] Provide a complete durable, filterable event stream distinct from the witness journal; the separate fsynced JSONL stream now projects Docker's core event wire shape with nanosecond timestamps, persisted method/path/status/scope actor attributes, label selectors, rejects corrupt journal records and invalid time bounds, supports type/action/scope/time/container/image/network/volume resource filters and Docker's JSON `filters` query form, and provides `/events` socket coverage including chunked `follow=1` streaming until client disconnect; full event-attribute parity remains open.
...
- [ ] Wire anomaly detection, adaptive restart, and resource prediction outcomes into the runtime lifecycle with durable per-container persistence. Anomaly, predictive-OOM, and adaptive-restart decisions now use a shared durable JSONL logger under the configured runtime directory by default (or `FERROCRATE_AI_AUDIT_LOG`), serialize concurrent container appenders under process and OS file locks, and sync the parent directory after each record; successful adaptive restarts emit `ai_restart_applied` receipts with delay/PID evidence, bounded `ai_restart_failed` receipts record command-build, spawn, PID-state, and ownership-release failures, and stale-PID recovery emits `ai_reconciliation_applied`; adaptive restart honors an AI `DoNotRestart` decision and is disabled by `FERROCRATE_AI=0`, while kernel-effect reconciliation persistence remains open.
