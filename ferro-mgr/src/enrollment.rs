...
use crate::{
    pki::{unix_now, CertificateAuthority, PkiError},
    store::{Enrollment, ManagerStore, ScopedToken, StoreError},
};
...
impl EnrollmentService {
    ...
    pub fn create_scoped_token(
        ...
        let token = self.store.create_token(ScopedToken {
            secret,
            expected_node,
    ...
    pub fn enroll(&self, token: &str, enrollment: Enrollment) -> Result<(), EnrollmentError> {
        let bytes = URL_SAFE_NO_PAD
            .decode(token)
            .map_err(|_| EnrollmentError::MalformedToken)?;
        let secret: [u8; 32] = bytes
            .try_into()
            .map_err(|_| EnrollmentError::MalformedToken)?;
        self.store
            .register_node_with_token(&secret, enrollment, unix_now())?;
        Ok(())
    }
    ...
    pub fn enroll_csr(
        ...
        let certificate =
            authority.sign_node_csr(csr_pem, &self.cluster_id, &enrollment.node_id)?;
