...
| Windows 11 via WSL2 backend | Qualified engine-start evidence only: WSL daemon health and a tokenized loopback bridge were observed; the non-interactive web process did not remain alive, so action acceptance remains pending | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): re-attempt 6 |
| macOS Tahoe via Linux VM backend | Host bridge blocked: nested HV and QEMU/HVF start a clean guest, but port 2222 closes during SSH key exchange with the regenerated unencrypted identity | [`VM-ACCEPTANCE-2026-08-25.md`](desktop/VM-ACCEPTANCE-2026-08-25.md): re-attempt 6; no bridge/runtime claim |
