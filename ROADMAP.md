...
- [ ] Add rootless contexts, socket discovery, diagnostics, logs, and explicit unsupported-feature messaging; Linux doctor now reports mapping/socket context, accepts only actual Unix sockets (rejecting regular-file/symlink decoys), and the CLI persists validated named contexts with context inspect availability reporting. Selected remote Unix-socket contexts now route images, containers, inspect, logs, non-streaming stats, network/volume listing, pause, unpause, stop, kill, restart, and remove through the Docker-compatible transport (docs/evidence/verification/2026-08-16-remote-context-routing.md); other mutating/streaming daemon mappings and full rootless qualification remain open.
