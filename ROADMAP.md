...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `af59978`
...
- [ ] Replace the remaining `sled` backends to remove `RUSTSEC-2025-0057` (`fxhash`) and `RUSTSEC-2024-0384` (`instant`), including migration and backward-compatibility tests. Volume, image, CRI metadata/replay, Compose replay, and the active container runtime now use SQLite with tested legacy imports and retained rollback copies; workspace/all-target serial tests pass against the active SQLite runtime, while the witness journal remains the final active sled backend and dependency removal still requires its cutover.
...
- [ ] Ship rootless installer and upgrade flows for subordinate IDs, mapping helpers, cgroup delegation, user services, and slirp networking; `scripts/rootless-install.sh` now provides non-privileged install and explicit `--upgrade` mode with atomic binary/unit replacement, prerequisite diagnostics, and dry-run mode, while helper provisioning and full networking qualification remain open.
...
- [ ] Provide a complete durable, filterable event stream distinct from the witness journal; the separate fsynced JSONL stream now projects Docker's core event wire shape with nanosecond timestamps and actor status/http-status attributes, rejects corrupt journal records and invalid time bounds, supports type/action/time/resource filters and Docker's JSON `filters` query form, and provides `/events` socket coverage including chunked `follow=1` streaming until client disconnect; full event-attribute parity remains open.
