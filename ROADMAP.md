...
- [ ] Test durable sandbox/container recovery after process termination at each intent/effect/store boundary; CRI metadata now uses transactional SQLite with legacy JSON import, no-root reopen coverage verifies persisted `Exited` state, exit code, and reason, the real OCI socket fixture restarts the CRI daemon while a workload is running and verifies recovered identity/state, and subprocess fixtures now hard-kill the actual `ferro-cri` binary after each durable sandbox/container lifecycle transition (`docs/evidence/verification/2026-08-16-cri-operation-recovery.md`). Mid-operation intent/effect/store fault injection and kernel-effect reconciliation remain open.
