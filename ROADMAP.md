...
**Maturity:** mature core with a broad, partially production-qualified platform · **Last updated:** 2026-08-16 · HEAD `cc2885a`
...
- [ ] Provide a complete durable, filterable event stream distinct from the witness journal; the separate fsynced JSONL stream now projects Docker's core event wire shape with nanosecond timestamps, type/action/time/resource filters, Docker's JSON `filters` query form, and `/events` socket coverage, including chunked `follow=1` streaming until client disconnect; full event-attribute parity remains open.
