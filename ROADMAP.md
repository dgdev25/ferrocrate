...
- [ ] Complete high-value Docker container, image, network, volume, streaming, wait, and event endpoints while preserving authorization and witness receipts; `/containers/{id}/wait` now honors the Docker timeout query (including zero=infinite), `/containers/{id}/logs` supports Docker `tail` and chunked `follow=1`, and `/images/prune` is authorized and wired, while broader streaming and endpoint parity remain open.
