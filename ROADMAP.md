...
- [ ] Ship rootless installer and upgrade flows for subordinate IDs, mapping helpers, cgroup delegation, user services, and slirp networking; `scripts/rootless-install.sh` now provides non-privileged install and explicit `--upgrade` mode with atomic binary/unit replacement, subordinate-ID/cgroup-v2/user-namespace/runtime-dir diagnostics, optional `--strict` fail-before-mutation gating, dry-run mode, collision checks, and systemd-unit path validation, with repeatable coverage in `scripts/test-rootless-install.sh`; helper provisioning and full networking qualification remain open.
