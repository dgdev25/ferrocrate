...
- [ ] Complete high-value Docker container, image, network, volume, streaming, wait, and event endpoints while preserving authorization and witness receipts; image history, container list `all`/`limit`/`since`/`before` filters with deterministic newest-first ordering, image-list reference/time filters, authorized image tagging, network/volume inspect/list/prune, `/containers/{id}/wait` timeout, pause/unpause, `/containers/{id}/top`, logs `tail`/chunked `follow=1`, and chunked stats streaming are wired, while broader streaming and endpoint parity remain open.
