...
- [ ] Implement real model/version routing, training-data collection, online-learning safeguards, and rollback gates. The training pipeline now atomically publishes version metadata and fails closed on duplicate/ambiguous versions or missing active artifacts; newly trained/imported artifacts are digest-bound and resolved through a fail-closed active-model router (`docs/evidence/verification/2026-08-16-ai-model-routing.md`); runtime lifecycle routing, provenance-bound data collection, online-learning safeguards, and policy-level rollback gates remain open.
