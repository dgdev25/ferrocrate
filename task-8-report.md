...
- Intent checkpoints are fsynced after bridge, WireGuard, forwarding, individual address/route, stale cleanup, endpoint link/master/netns, and deletion effects. Each checkpoint retains an ordered observed-phase trail. A checkpoint write fault leaves the preceding durable phase and records the granted result as outcome-unknown; restart resolves from complete live receipts rather than trusting the phase label.
- Deterministic address and route effects are additive per item, enabling partial-list interruption and exact receipt mismatch detection. A restart test injects a named phase-persistence failure after a live bridge effect and proves recovery commits the exact desired receipt without replay.
- The former monolithic server was separated into configuration, dispatch, execution, state/reconciliation, and deterministic-test modules.
- Nonterminal mutation intents now form an immediate same-process exclusion boundary for both overlay and endpoint identities, preventing a later grant or disabled request from overwriting recovery evidence. A pre-effect intent-write failure restores the prior in-memory record.
- Disabled desired-state validation rolls back its reserved policy revision when mutation setup fails, so an identical signed revision can be retried instead of becoming spuriously stale.
- Ownership journal commits use a process-exclusive lock, unique `O_EXCL|O_NOFOLLOW` 0600 temporary files, file `sync_all`, atomic rename, and parent-directory `sync_all`; failed temporary files are removed and stale fixed-name files cannot wedge later commits.
- Ownership state is now a bounded versioned binary frame binding an independently generated journal ID, monotonic generation, payload length, and SHA-256 checksum. Restart rejects truncated, oversized, modified, or wrong-version primaries before decoding JSON. Unique orphan temporary files are never trusted as committed state; they are renamed to digest-addressed quarantine evidence and the last complete primary remains authoritative.
- Test-support fault injection reaches the actual journal open, create, write, file-fsync, rename, and directory-fsync boundaries. The table-driven reopen test proves pre-rename faults retain generation N while a post-rename directory-fsync ambiguity exposes only the complete checksummed generation N+1; neither path exposes partial state or wedges the next writer.
- Production-owned addresses and routes use Linux protocol marker 186. Live observation hashes only entries carrying that marker: unrelated host entries are excluded, while any stale or substituted FerroCrate-managed entry remains in the exact managed set and forces mismatch/quarantine. Deletes target the same marked route ownership domain.
- Endpoint detach execution is isolated from the overlay dispatcher and treats ownership-persist failure as outcome-unknown even when the kernel link was already absent. Frame/handshake validation moved to dispatch. `server_execute.rs` is 492 lines and `server_state.rs` is 490 lines.
- A real-handler disabled-mode matrix now executes create, update, bridge-only→WireGuard, WireGuard→bridge-only, and final removal through signed desired envelopes. It injects every applicable bridge/WireGuard/forwarding/address/route/stale-cleanup checkpoint plus ownership persistence, hard-reopens kernel and ownership journals, requires exact authoritative ownership or quarantine, and verifies replay cannot wedge. This exposed and fixed missing legacy address ownership persistence.
- A separate granted-operation matrix signs exact child grants and manager envelopes, consumes them through the real `GrantLedger` and `NetdServer`, and covers overlay create/update/mode transitions/remove plus endpoint attach (link creation, bridge master, netns move) and cleanup-only detach. Every applicable named checkpoint and ownership-persist boundary is injected. Immediate results cannot be `Failed` after effects; hard reopen must expose exact topology with consistent receipts or explicit quarantine, and replay adds no receipt or kernel mutation.
