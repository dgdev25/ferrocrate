...
- [ ] Qualify rootless networking, volumes, image pulls, and CRI constraints on each supported row; record strict prerequisite failures. Rootless bridge networking and image pulls now pass on the Ubuntu 26.04 row through the opt-in `slirp4netns` path (`docs/evidence/verification/2026-08-16-rootless-slirp.md`), with IPv6 command support opt-in but live traffic, port mappings, rootless volumes, CRI, and other host rows remaining open.
